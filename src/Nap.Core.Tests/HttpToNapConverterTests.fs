...
let ``Spec http-convert-output: one nap per request`` () =
    Assert.Equal(3, (parseAndConvert "GET https://a.com\n\n###\nPOST https://b.com\nContent-Type: application/json\n\n{\"n\":1}\n\n###\nDELETE https://c.com\n").GeneratedFiles.Length)
...
let ``Spec http-convert-env: public env generates named napenv`` () =
    match convertEnvJson """{"dev":{"host":"localhost:8080","token":"abc"},"prod":{"host":"api.example.com"}}""" false with
    | Ok files ->
...
let ``Spec http-convert-env: private env generates napenv.local`` () =
    ...
    | Ok files -> Assert.Equal(".napenv.local", fst files[0]); Assert.Contains("secret = \"s3cret\"", snd files[0])
    | Error e -> failwith e
