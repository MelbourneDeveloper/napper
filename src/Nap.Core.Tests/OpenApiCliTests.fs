...
let private generateFromUrl (url: string) : Nap.Core.OpenApiGenerator.GenerationResult =
    let spec = httpClient.GetStringAsync(url) |> Async.AwaitTask |> Async.RunSynchronously
    match Nap.Core.OpenApiGenerator.generate spec with
    | Ok result -> result
...
let ``OpenApiGenerator.generate produces vars for all path param endpoints`` () =
    ...
    | Ok result ->
        let paramFiles =
            result.NapFiles |> List.filter (fun f ->
                f.Content.Contains("{{petId}}") || f.Content.Contains("{{orderId}}") || f.Content.Contains("{{username}}"))
...
let ``Beeceptor URL generates cart endpoint`` () =
    let result = generateFromUrl BeeceptorSpecUrl
    let hasCart = result.NapFiles |> List.exists (fun f -> f.Content.Contains BeeceptorCartPath)
    Assert.True(hasCart, "Must have cart endpoint from beeceptor spec")

[<Fact>]
let ``Beeceptor URL generates checkout endpoint`` () =
    let result = generateFromUrl BeeceptorSpecUrl
    let hasCheckout = result.NapFiles |> List.exists (fun f -> f.Content.Contains BeeceptorCheckoutPath)
    Assert.True(hasCheckout, "Must have checkout endpoint from beeceptor spec")

[<Fact>]
let ``Beeceptor URL generates orders endpoint`` () =
    let result = generateFromUrl BeeceptorSpecUrl
    let hasOrders = result.NapFiles |> List.exists (fun f -> f.Content.Contains BeeceptorOrdersPath)
    Assert.True(hasOrders, "Must have orders endpoint from beeceptor spec")

[<Fact>]
let ``Beeceptor URL generates addresses endpoint`` () =
    let result = generateFromUrl BeeceptorSpecUrl
    let hasAddresses = result.NapFiles |> List.exists (fun f -> f.Content.Contains BeeceptorAddressesPath)
...
let ``Beeceptor URL generates bearer auth on secured endpoints`` () =
    let result = generateFromUrl BeeceptorSpecUrl
    let bearerFiles = result.NapFiles |> List.filter (fun f -> f.Content.Contains "Authorization = Bearer {{token}}")
