...
suite("explorerProvider — appendStepToPlaylist", () => {
  ...
  test("inserts before next section when [steps] is followed by another section", () => {
    ...
    assert.ok(
      newStepIdx < scriptsIdx,
      "new step must be inserted before the [scripts] section"
    );
  });
});
...
suite("explorerProvider — updatePlaylistName", () => {
  test("replaces existing name line", () => {
    ...
    assert.ok(
      !result.includes("old-name"),
      "old name must be replaced"
    );
    assert.ok(
      result.includes("step.nap"),
      "non-name lines must be preserved"
    );
  });
